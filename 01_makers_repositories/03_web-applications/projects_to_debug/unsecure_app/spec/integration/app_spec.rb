...
describe Application do
  ...
  context 'POST /hello' do
    ...
    it 'should return "invalid characters detected" if params is funny looking' do
      response = post('/hello', name: '<script>')

      expect(response.status).to eq(200)
      expect(response.body).to include('Invalid characters detected')
    end
  end
end